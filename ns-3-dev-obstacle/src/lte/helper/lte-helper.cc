...
for (std::vector<ObjectFactory>::const_iterator i = m_propagationLoss.begin (); i != m_propagationLoss.end (); ++i)
  ...
  m_hndStats = CreateObject<HandoverStatsCalculator> ();
  Object::DoInitialize ();
...
LteHelper::EnableLogComponents (void)
{
  ...
  LogComponentEnable ("HandoverStatsCalculator", LOG_LEVEL_ALL);
...
void
LteHelper::EnableHandoverTraces (void)
{
EnableMesRcvTraces ();  
}

void
LteHelper::EnableMesRcvTraces (void)
{
  NS_LOG_FUNCTION_NOARGS ();
  Config::Connect("/NodeList/*/DeviceList/*/LteEnbRrc/RecvMeasurementReport",
                   MakeBoundCallback (&HandoverStatsCalculator:: RecvMeasurementReportCallback, m_hndStats));
}
...
void
LteHelper::AddPropagationLoss (std::string type,
