...
bool
Box::Clipline_x (const Vector &position_a,const Vector &position_b,float &f_low,float &f_high) const
{
...
// f_dim_low and f_dim_high are the results we're calculating for this current dimension.
    ...
    NS_LOG_INFO ("Box dimentions " << this->xMin<< "," <<this->yMin<< "," <<this->zMin<< "," <<this->xMax<< "," <<this->yMax<< "," <<this->zMax);
    f_dim_low=(this->xMin-position_a.x)/(position_b.x-position_a.x);
    f_dim_high=(this->xMax-position_a.x)/(position_b.x-position_a.x);

    // Make sure f_dim_low is less than f_dim_high
...
bool
Box::Clipline_y(const Vector &position_a,const Vector &position_b,float &f_low,float &f_high) const
{
    ...
    f_dim_low=(this->yMin-position_a.y)/(position_b.y-position_a.y);
    f_dim_high=(this->yMax-position_a.y)/(position_b.y-position_a.y);
...
bool
Box::Clipline_z(const Vector &position_a,const Vector &position_b,float &f_low,float &f_high) const
{
    ...
    f_dim_low=(this->zMin-position_a.z)/(position_b.z-position_a.z);
    f_dim_high=(this->zMax-position_a.z)/(position_b.z-position_a.z);
...
bool
Box::IsIntersect(const Vector &position_a,const Vector &position_b) const
{

if ( (IsInside(position_a)||IsInside(position_b)))
    {
      return true;
   }
  ...
  Xintersection=Clipline_x(position_a,position_b,f_low,f_high);
   ...
   Yintersection=Clipline_y(position_a,position_b,f_low,f_high);
    ...
    Zintersection=Clipline_z(position_a,position_b,f_low,f_high);
